...
$Branch = "main"
$InstallerUrl = "https://raw.githubusercontent.com/rylena/sshdesk-go/main/scripts/install.ps1"
...
try {
    ...
    # The Go build is one static binary; command names are .cmd wrappers.
    $Binary = Join-Path $TemporaryDirectory "sshdesk.exe"
    $Asset = "sshdesk-windows-amd64.exe"
    Invoke-WebRequest "https://github.com/$Repository/releases/latest/download/$Asset" `
        -OutFile $Binary -UseBasicParsing
    $Checksum = (Invoke-WebRequest `
    ...
    if ($Actual -ne $Expected) {
        throw "SHA-256 mismatch for the SSHDESK release binary"
    }
