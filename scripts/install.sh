...
install_prerequisites() {
    command -v sshd >/dev/null 2>&1 || [ -x /usr/sbin/sshd ] || {
        say "Installing the OpenSSH server package..."
        if command -v apt-get >/dev/null 2>&1; then
            ${as_root} apt-get update
            ${as_root} apt-get install -y openssh-server
        elif command -v dnf >/dev/null 2>&1; then
            ${as_root} dnf install -y openssh-server
        elif command -v yum >/dev/null 2>&1; then
            ${as_root} yum install -y openssh-server
        elif command -v pacman >/dev/null 2>&1; then
            ${as_root} pacman -Sy --needed --noconfirm openssh
        elif command -v zypper >/dev/null 2>&1; then
            ${as_root} zypper --non-interactive install openssh
        elif command -v apk >/dev/null 2>&1; then
            ${as_root} apk add openssh-server
        else
            fail "install the OpenSSH server, then rerun this command"
        fi
    }
}
...
# The Go GNOME backend drives GStreamer through the gst-launch-1.0 executable,
# so readiness is the executable plus the PipeWire source plugin.
...
install_gnome_streaming_support() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        ${as_root} apk add \
            gstreamer gstreamer-tools gst-plugins-base gst-plugin-pipewire
    else
        fail "install GStreamer, its tools, and its PipeWire plugin, then rerun"
    fi
    gnome_streaming_is_ready || \
...
# The scripts still ship in the source archive; SSHDESK_SOURCE_DIR points at a
# local Go checkout instead.
download_project() {
    if [ -n "${SSHDESK_SOURCE_DIR-}" ]; then
        project_directory="${SSHDESK_SOURCE_DIR}"
        [ -f "${project_directory}/go.mod" ] || fail "invalid SSHDESK_SOURCE_DIR"
        return
    fi
...
# The installed artifact is one static binary. Release assets are named
# sshdesk-<os>-<arch> with a <asset>.sha256 sidecar on GitHub releases.
resolve_binary() {
    if [ -n "${SSHDESK_BINARY-}" ]; then
        binary_path="${SSHDESK_BINARY}"
        [ -x "${binary_path}" ] || fail "SSHDESK_BINARY is not executable: ${binary_path}"
        return
    fi
    if [ -n "${SSHDESK_SOURCE_DIR-}" ]; then
        binary_path="${SSHDESK_SOURCE_DIR}/sshdesk"
        if [ ! -x "${binary_path}" ]; then
            command -v go >/dev/null 2>&1 || \
                fail "no built sshdesk binary in SSHDESK_SOURCE_DIR; run 'go build -o sshdesk ./cmd/sshdesk' there first"
            say "Building the SSHDESK binary from SSHDESK_SOURCE_DIR..."
            binary_path="${temporary_directory}/sshdesk"
            (cd "${SSHDESK_SOURCE_DIR}" && go build -o "${binary_path}" ./cmd/sshdesk)
        fi
        return
    fi
    binary_path="${temporary_directory}/sshdesk"
    download_release_binary "${binary_path}"
}
...
download_release_binary() {
    ...
    asset="sshdesk-${asset_os}-${asset_arch}"
    say "Downloading the SSHDESK ${asset} release binary..."
    curl -fsSL \
        "https://github.com/${REPOSITORY}/releases/latest/download/${asset}" \
        -o "${destination}"
    curl -fsSL \
        "https://github.com/${REPOSITORY}/releases/latest/download/${asset}.sha256" \
    ...
    [ "${actual}" = "${expected}" ] || \
        fail "SHA-256 mismatch for the SSHDESK ${asset} release binary"
